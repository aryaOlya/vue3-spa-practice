<template>
  <section class="col-md-6" style="margin: 0 auto">
    <h2 class="mb-3 text-center">Update Post</h2>
...
axios.get(`https://jsonplaceholder.typicode.com/posts/${useRoute().params.id}`)
    .then(response=> {
      ...
      bodyValue.value = response.data.body
      loading.value=false
    })
  ...
  axios.put(`https://jsonplaceholder.typicode.com/posts/${postId}`,{
    ...
    id:postId
      ...
      .then(response => {
        ...
        loading.value = false
        Swal.fire(`title ${currentTitle.value} updated successfully`)
      })
      .catch(()=>{
        console.log('error')
        loading.value = false
